...
unsigned char
GetCurrentPageNbr(void)
{
	unsigned char currentPage;

	for(currentPage = 0;
		(currentPage < NUM_DATA_EE_PAGES) &&
		(GetPageStatus(0, currentPage, STATUS_CURRENT) == PAGE_NOT_CURRENT);
		currentPage++) {}
	return currentPage;
}

unsigned int
GetCurrentPageSts(void)
{
	...
	TBLPAG = DEE_PAGE_TBL(0, currentPage);
	statusOffset = DEE_PAGE_OFFSET(0, currentPage);

	return ReadPMLow(statusOffset);
}
/************************************************************************
ErasePage
